...
export type PathKeys<T> = {
    [K in StringKeys<T>]: T[K] extends Procedure<any, any> ? K : T[K] extends object ? `${K}.${PathKeys<T[K]>}` : never;
}[StringKeys<T>];
export type ProcedureAtPath<T, P extends string> = P extends `${infer K}.${infer Rest}` ? K extends StringKeys<T> ? ProcedureAtPath<T[K], Rest> : never : P extends StringKeys<T> ? T[P] extends Procedure<any, any> ? T[P] : never : never;
export type InputAtPath<R extends RouterDef, P extends PathKeys<R>> = InferInput<ProcedureAtPath<R, P>>;
export type OutputAtPath<R extends RouterDef, P extends PathKeys<R>> = InferOutput<ProcedureAtPath<R, P>>;
